...
from algobip44.utils import get_ALGO_addr, convert_ALGO_account_to_checksum
from algobip44 import Wallet
...
def test_utils():
    ...
    assert get_ALGO_addr(pk) == get_ALGO_addr(bytes.fromhex(pk))
    assert get_ALGO_addr(pk) == addr
    assert convert_ALGO_account_to_checksum(addr.lower()) == addr

    addr = "0x0Fd60495d705F4Fb86e1b36Be396757689FbE8B3"
    assert convert_ALGO_account_to_checksum(addr.lower()) == addr

def test_ALGO_wallet():
    w = Wallet(MNEMONIC)

    sk, pk = w.derive_account("ALGO")
    ...
    assert get_ALGO_addr(pk_64bytes) == expected_addr
    assert get_ALGO_addr(pk) == expected_addr
    assert get_ALGO_addr(bytes.fromhex(pk_64bytes.hex())) == expected_addr
